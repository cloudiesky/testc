...
int init_cfg(cfg_t **config, const char FnameCfg[])
{
  ...
  // initial parameters
  FidCfgFile = fopen(FnameCfg,"r");
  while(1){
    fscanf(FidCfgFile, "%s = %f", ParamName, &ParamVal);
    if (feof(FidCfgFile)) break;
    printf("%s = %f\n",  ParamName, ParamVal);

    if  (strcmp(ParamName, "Mod") == 0) {
      (*config)->Mod = (int)(ParamVal);
      debug(V_DEBUG, "config->Mod  %d\n", (*config)->Mod);
    }
    else if (strcmp(ParamName, "Len") == 0) {
      (*config)->Len = (int)(ParamVal);
    }
    else if (strcmp(ParamName, "InSrc") == 0) {
      (*config)->InSrc = (int)(ParamVal);
    }
    else if (strcmp(ParamName, "FftTyp") == 0) {
      (*config)->FftTyp = (int)(ParamVal);
    }
    else
      assert(0);
  }
